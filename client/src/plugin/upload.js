function upload(Vue, data, callbackSuccess, callbackFail) {
    Vue.prototype.$axios.getUploadToken().then(res => {
        if (typeof data === 'string'){
            ...
            Vue.prototype.$axios.uploadBase64File(data, token).then(res => {
                if (res.status === 200){
                    callbackSuccess && callbackSuccess({
                        data: res.data,
                        result_url: `http://mafei.ostqa.cn/${res.data.key}`
                    })
                }
            }).catch((error) => {
                console.log(error)
            ...
            data.append('key', `reader${Date.now()}${Math.floor(Math.random() * 100)}`)
            ...
            formdata.append('key', `reader${Date.now()}${Math.floor(Math.random() * 100)}.${data.file.type.split('/')[1]}`)
            // 获取到凭证之后再将文件上传到七牛云空间
