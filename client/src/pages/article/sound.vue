<template>
  <div class="sound">
      ...
      <div class="container">
        ...
        <div class="operate">
          <div
            ...
            @click="handleCollect(soundArticleInfo)"
          ></div>
          <div class="share"></div>
    ...
    <div class="container" id="info" style="overflow-y: scroll;">
      ...
      <div
        class="author"
      >主播 / {{(soundArticleInfo.author && soundArticleInfo.author) ? soundArticleInfo.author.username : '我叫这个名字'}}</div>
      <div class="cover">
      ...
      <div class="author-info">
        ...
        <div class="box">
          <div class="main">
            <div
              class="avatar"
              @click="toUser(soundArticleInfo.author)"
              :style="{background: `url(${(soundArticleInfo.author && soundArticleInfo.author) ? soundArticleInfo.author.avatar : ''}) no-repeat center`, backgroundSize: `${soundArticleInfo.author ? '100% auto' : ''}`}"
            ></div>
            <div class="info">
              <div
                class="name"
              >{{(soundArticleInfo.author && soundArticleInfo.author) ? soundArticleInfo.author.username : 'TA'}}</div>
              <div
                class="description"
              >{{(soundArticleInfo.author && soundArticleInfo.author) ? soundArticleInfo.author.description : 'TA'}}</div>
            </div>
          </div>
          <div
            class="follow"
            :class="{active: isFollowed}"
            @click="handleFollow(soundArticleInfo.author)"
          >{{isFollowed ? '已关注' : '关注'}}</div>
        </div>
      </div>
      <div class="comment">
        <div class="sub">评论</div>
        <div class="main" v-for="item in soundArticleInfo.comment" v-bind:key="item._id">
          <div class="info">
            <div class="top">
              <div class="left">
                <div
            ...
            <div class="bottom">
              <div
                class="delComment"
                v-if="user_info._id === soundArticleInfo.author._id"
        ...
        <div
          class="noComment"
          v-if="soundArticleInfo.comment ? !soundArticleInfo.comment.length : true"
        >还没有评论哦～</div>
      </div>
    </div>
...
export default {
  ...
  created() {
    ...
    this.$axios
      .getSoundArticleById(this.$route.params.id)
      .then(res => {
        ...
        this.$axios
          .getFriendShipById({
            ...
            to_user: this.soundArticleInfo.author._id
          })
          .then(res => {
