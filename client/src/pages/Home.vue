<template>
  <div class="home">
    <m-header style="position: relative;"></m-header>
    <div class="container">
      <!-- <swiper class="swiper-box" ref="swiper" v-if="list.length">
        <swiper-item v-for="(item,index) in list" :key="index">
          <img :src="item.img" />
        </swiper-item>
      </swiper>-->
      <div class="classify">
        <div class="reading" @click="toReadingList">
          <span>阅读</span>
        </div>
        <div class="film" @click="toFilmList">
          <span>影视</span>
        </div>
        <div class="music" @click="toMusicList">
          <span>音乐</span>
        </div>
        <div class="sound" @click="toSoundList">
          <span>电台</span>
        </div>
      </div>
      <div
        style="background-color: rgb(255,255,255); font-size: x-large;color: rgba(0,0,0,0.7);padding-top: 16px;"
      >- 置顶推荐 -</div>
      <div class="banner">
        <div class="film" @click="toFilm">
          ...
          <div class="info">
            ...
            <div class="view-count">
              <div class="icon"></div>
              <div class="txt">浏览量:&nbsp;{{topFilmInfo.views_count}}</div>
            </div>
          </div>
        </div>
        <div class="music" @click="toMusic">
          ...
          <div class="info">
            ...
            <div class="music-info">
              <div
                class="image"
                :style="{backgroundImage: `url(${topMusicInfo.music_info ? topMusicInfo.music_info.cover : topMusicInfo.cover_url })`, backgroundSize: '100% auto'}"
              ...
              >{{topMusicInfo.music_info ? topMusicInfo.music_info.name : ''}} / {{topMusicInfo.music_info ? topMusicInfo.music_info.singer : ''}}</div>
            </div>
            <div class="abstract">{{topMusicInfo.abstract}}</div>
            <div class="view-count">
              <div class="icon"></div>
              <div class="txt">浏览量:&nbsp;{{topMusicInfo.views_count}}</div>
            </div>
          </div>
        </div>
        <div class="reading" @click="toReading">
          ...
          <div class="info">
            ...
            <div class="view-count">
              <div class="icon"></div>
              <div class="txt">浏览量:&nbsp;{{topReadingInfo.views_count}}</div>
            </div>
          </div>
        </div>
        <div class="sound" @click="toSound">
          ...
          <div class="info">
            ...
            <div class="view-count">
              <div class="icon"></div>
              <div class="txt">浏览量:&nbsp;{{topSoundInfo.views_count}}</div>
            </div>
          </div>
        </div>
      </div>
    </div>
    <m-footer style="position: fiex;bottom:0;"></m-footer>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      list: [
        { img: "http://mafei.ostqa.cn/w.jpg" },
        { img: "http://mafei.ostqa.cn/www.jpg" },
        { img: "http://mafei.ostqa.cn/ww.jpg" }
      ],
      musicPlay: false,
      musicFirst: true,
      soundPlay: false,
  ...
  created() {
    ...
    Promise.all(
      ["ReadingArticle", "FilmArticle", "MusicArticle", "SoundArticle"].map(
        item => {
      ...
      .then(res => {
        this.preload([
          `${this.topReadingInfo.cover_url}`,
          `${this.topFilmInfo.cover_url}`,
          `${this.topMusicInfo.cover_url}`,
          `${this.topSoundInfo.cover_url}`
        ]).done(() => {
  ...
  mounted() {
    ...
    container.style.height = `${document.body.clientHeight - 98}px`;
...
.classify div {
  ...
  min-width: 210px;
  min-height: 280px;
  background-image: url("../assets/images/sound-thumb.jpg");
...
.classify .reading {
  background-image: url("../assets/images/reading-thumb.jpg");
}
.classify .film {
  background-image: url("../assets/images/film-thumb.jpg");
}
.classify .music {
  background-image: url("../assets/images/music-thumb.jpg");
}
.classify .sound {
  background-image: url("../assets/images/sound-thumb.jpg");
}
