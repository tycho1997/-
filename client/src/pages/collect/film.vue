<template>
  <div class="film-list">
    <list-header title="影视文章"></list-header>
    <div class="container">
      <div class="no center" v-if="!filmArticleList.length">
        <div>無</div>
      </div>
      <div
        class="banner"
        v-else
        v-for="item in filmArticleList"
        v-bind:key="item._id"
        @click="toArticleDetail(item)"
      >
        <img class="center" :src="item.cover_url" alt />
        <div class="main">
          <div class="title">{{item.title}}</div>
          <div class="film_name">《{{item.film_info ? item.film_info.name : ''}}》</div>
          <div class="info">
            <div class="time">{{moment(item.update_time,`YYYY/MM/DD`)}}</div>
            <div class="author">©&nbsp;️{{item.author.user_id ? item.author.user_id.username : ''}}</div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import ListHeader from "../common/ListHeader";
import mixin from "../../mixins";
import moment from "moment";
export default {
  name: "film-list",
  mixins: [mixin],
  components: {
    ListHeader
  },
  data() {
    return {
      filmArticleList: []
    };
  },
  created() {
    this.filmArticleList = this.$route.params[1];
  },
  watch: {
    $route(to, from) {
      if (to.name === "FilmCollect" && from.path.indexOf("film") > -1) {
        this.$router.push("/me");
      }
      this.filmArticleList = to.params["1"];
    }
  },
  methods: {
    goBack() {
      this.setTransitionName("move-left-to-right");
      this.$router.go(-1);
    },
    toArticleDetail(item) {
      this.setTransitionName("move-right-to-left");
      this.$router.push({
        path: `/film/${item._id}`
      });
    }
  }
};
</script>

<style lang="scss" scoped>
.film-list {
  position: absolute;
  left: 0;
  top: 0;
  width: 100%;
  height: 100vh;
  display: flex;
  flex-direction: row;
  flex-wrap: wrap;
  justify-content: space-around;
  .container {
    width: 640px;
    overflow-y: scroll;
    margin-top: 128px;
    .no {
      font-size: 50px;
      color: #666;
    }
    .banner {
      width: 670px;
      height: 300px;
      border-radius: 10px;
      margin: 0 auto;
      box-shadow: 0 0 15px 0px #c4c4c4;
      margin-bottom: 30px;
      position: relative;
      overflow: hidden;
      &:last-child {
        margin-bottom: 0;
      }
      img {
        width: 100%;
        -webkit-filter: blur(1px);
        -moz-filter: blur(1px);
        -o-filter: blur(1px);
        -ms-filter: blur(1px);
        filter: blur(1px);
      }
      .main {
        position: absolute;
        left: 0;
        top: 0;
        width: 100%;
        height: 100%;
        z-index: 10;
        display: flex;
        justify-content: center;
        align-items: center;
        flex-direction: column;
        .title {
          width: 98%;
          margin: 0 auto;
          font-size: 30px;
          font-weight: 400;
          color: #fff;
          word-break: break-all;
          word-wrap: break-word;
          display: -webkit-box;
          -webkit-box-orient: vertical;
          -webkit-line-clamp: 2; //这里是在第二行有省略号
          overflow: hidden;
        }
        .film_name {
          margin-top: 10px;
          font-size: 25px;
          font-weight: 300;
          color: #fff;
        }
        .info {
          position: absolute;
          right: 0;
          bottom: 20px;
          width: 100%;
          padding: 0 20px;
          font-size: 20px;
          text-align: right;
          color: #fff;
        }
      }
    }
  }
}
</style>
