  ...
  <div class="music-collect">
    ...
    <div class="container">
      ...
      >
        <div
          class="left"
          :style="{background: `url(${item.music_info ? item.music_info.cover : ''}) no-repeat center`, backgroundSize: '100% auto'}"
        ></div>
        <div class="right">
          <div class="title">{{item.title}}</div>
          <div
            class="music_info"
          >{{item.music_info ? item.music_info.name : '未知'}} / {{item.music_info.singer ? item.music_info.singer: '未知'}}</div>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    this.musicArticleList = this.$route.params[1];
  },
  watch: {
    $route(to, from) {
      ...
      this.musicArticleList = to.params["1"];
    }
  },
  methods: {
    goBack() {
