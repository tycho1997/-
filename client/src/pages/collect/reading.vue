      ...
      >
        ...
        <div class="main">
          ...
          <div class="info">
            ...
            <div class="author">©&nbsp;️{{item.author ? item.author.username : ''}}</div>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.readingArticleList = this.$route.params["1"];
    //console.log("this.$route", this.readingArticleList);
  },
  watch: {
    $route(to, from) {
      ...
      this.readingArticleList = to.params["1"];
    }
  },
  methods: {
    goBack() {
