<template>
  <div class="me">
    <div v-if="user_info.token" class="container">
      <div
        class="top"
        :style="{background: `url(${user_info.bg_url}) no-repeat center`, backgroundSize: '100% auto'}"
      >
        ...
        <div class="main">
          <div
            class="following"
            @click="toUserList({item: user_info.following_user, type: 'following'})"
          >
            <p>我的关注</p>
            <p class="num">{{user_info.following_user ? user_info.following_user.length : 0}}</p>
          </div>
          <div
            class="avatar centerX"
            :style="{background: `url(${user_info.avatar}) no-repeat center`, backgroundSize: '100% auto'}"
          ></div>
          <div
            class="follower"
            @click="toUserList({item: user_info.follower_user, type: 'follower'})"
          >
            <p>关注我的</p>
      ...
      <div class="my-reader">
        <div class="sub">My Reader</div>
        <div class="noreader" v-if="!imageArticleList.length">你还没有Reader，赶快去创建吧~</div>
        <div v-else class="banner" v-for="item in imageArticleList" v-bind:key="item._id">
...
export default {
  ...
  mounted() {
    ...
    container.style.height = `${document.body.clientHeight - 83}px`;
  ...
  created() {
    ...
    this.$axios
      .getImageArticleListByUserId(this.user_info._id)
      .then(res => {
  ...
  methods: {
    ...
    toImageCollect() {
      this.setTransitionName("move-right-to-left");
      let temp = {'1': this.user_info.collection_image_article};
      this.$router.push({
        name: "ImageList",
        params: temp
      });
    ...
    toReadingCollect() {
      this.setTransitionName("move-right-to-left");
      let temp = {'1': this.user_info.collection_reading_article};
      this.$router.push({
        name: "ReadingCollect",
        params: temp
      });
    ...
    toMusicCollect() {
      this.setTransitionName("move-right-to-left");
      let temp = {'1': this.user_info.collection_music_article};
      this.$router.push({
        name: "MusicCollect",
        params: temp
      });
    ...
    toFilmCollect() {
      this.setTransitionName("move-right-to-left");
      let temp = {'1': this.user_info.collection_film_article};
      this.$router.push({
        name: "FilmCollect",
        params: temp
      });
    ...
    toSoundCollect() {
      this.setTransitionName("move-right-to-left");
      let temp = {'1': this.user_info.collection_sound_article};
      this.$router.push({
        name: "SoundCollect",
        params: temp
      });
