<template>
  <div class="admin-list">
    ...
    >
      ...
      <el-table-column prop="role_name" label="用户类型" sortable show-overflow-tooltip></el-table-column>
      ...
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button
            ...
            plain
            circle
            @click="editUserInfo(scope.$index, adminList.docs)"
          >
          ...
          <el-button
            ...
            circle
            icon="el-icon-delete"
            @click="deleteAdmin(scope.$index, adminList.docs)"
          ></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog
      :title="`${adminInfoForm.data.username}的信息`"
      :visible.sync="adminInfoForm.show"
      center
    >
      <el-form
        :rules="rules"
        ref="adminInfoForm"
        :model="adminInfoForm.data"
        label-position="left"
        label-width="100px"
      ...
      >
        ...
        <el-form-item label="用户类型" prop="role_name">
          <el-select
            v-model="adminInfoForm.data.role_name"
            placeholder="请选择"
            :disabled="userInfo.role_name !== '超级管理员' || userInfo._id === adminInfoForm.data._id || adminInfoForm.data.role_name === '超级管理员'"
          >
            <el-option
...
export default {
  ...
  created() {
    this.userInfo = JSON.parse(window.localStorage.getItem("userInfo")).data;
  },
  mounted() {
  ...
  methods: {
    ...
    deleteAdmin(index, rows) {
      let rowData = rows[index];
      if (
        (this.userInfo.role_name === "超级管理员" &&
          rowData.role_name !== "超级管理员") ||
        (this.userInfo.role_name === "管理员" && rowData.role_name === "作者")
      ) {
        this.axios
          .deleteAdmin({ _id: rowData._id })
          .then(res => {
            this.getAdminList();
            this.$message({
    ...
    updateAdminInfo() {
      this.$refs["adminInfoForm"].validate(valid => {
        ...
        this.axios
          .updateAdminInfo(this.adminInfoForm.data)
          .then(res => {
