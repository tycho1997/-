<template>
  <div class="user-all-list">
    ...
    >
      ...
      <el-table-column prop="email" label="邮箱" show-overflow-tooltip sortable></el-table-column>
      <el-table-column prop="ip" label="上次登录IP" show-overflow-tooltip></el-table-column>
      <el-table-column prop="ip_location.city" label="上次登录地点" show-overflow-tooltip sortable></el-table-column>
      ...
      <el-table-column label="收藏">
        <el-table-column prop="collection_reading_article.length" label="阅读文章收藏数" sortable>
          <template slot-scope="scope">
            <el-button type="text">{{scope.row.collection_reading_article.length}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="collection_music_article.length" label="音乐文章收藏数" sortable>
          <template slot-scope="scope">
            <el-button type="text">{{scope.row.collection_music_article.length}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="collection_film_article.length" label="影视文章收藏数" sortable>
          <template slot-scope="scope">
            <el-button type="text">{{scope.row.collection_film_article.length}}</el-button>
          </template>
        </el-table-column>
        <el-table-column prop="collection_sound_article.length" label="电台文章收藏数" sortable>
          <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="100">
        <template slot-scope="scope">
          <el-button
            ...
            circle
            icon="el-icon-delete"
            @click="deleteUser(scope.$index, userList.docs)"
          ></el-button>
        </template>
      </el-table-column>
    </el-table>
