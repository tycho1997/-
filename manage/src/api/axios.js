...
instance.interceptors.request.use(
    config => {
        ...
        if (localStorage.getItem('token')) {
            if (config.url.indexOf('up-z2.qiniup.com/putb64') > -1) {
...
export default {
    ...
    * */
    ...
    uploadFile(formdata, domain = 'https://up-z2.qiniup.com', config = { headers: { 'Content-Type': 'multipart/form-data' } }) {
        console.log(domain)
        console.log(formdata)
        return instance.post(domain, formdata, config)
    },
    //根据获取到的上传凭证uploadToken上传base64到指定域
    uploadBase64File(base64, token, domain = 'https://up-z2.qiniup.com/putb64/-1', config = {
        headers: {
    ...
    * */
    ...
    getSummary() {
        return instance.post(`/api/getSummary`, {})
    },

    deleteAdmin(data){
        return instance.post(`/api/deleteAdmin`, data)
    },
    /*
    * user 接口
    * */
    ...
    * */
    ...
    deleteArticle(type, id) {
        switch (type) {
            case 'film':
                return instance.post(`/api/deleteFilmArticle`, {id: id});
            case 'music':
                return instance.post(`/api/deleteMusicArticle`, {id: id});
            case 'sound':
                return instance.post(`/api/deleteSoundArticle`, {id: id});
            case 'reading':
                return instance.post(`/api/deleteReadingArticle`, {id: id});
        }
    }
}