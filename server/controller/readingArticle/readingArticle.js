...
class ReadingArticle {
    ...
    async addReadingArticle(req,res,next){
        ...
        try {
            ...
            sendMail({
                to: admin.email,
                subject: `Reader | 你投稿的文章《${newReadingArticle.title}》，${newReadingArticle.status === 0 ? '以保存为草稿哦，还未发布哦~ 😕' : (newReadingArticle.status === 1 ? '正在审核当中，耐心等待哦~ 🤓' : (newReadingArticle.status === 2 ? '审核成功啦，真棒!! 👏 ' : (newReadingArticle.status === 3 ? '不好意思审核失败，可能你的文章还不适合我们，期待下次的合作~ 🤘' : (newReadingArticle.status === 4 ? '发布成功啦，正在 [阅读版块] 上线，去看看吧皮卡丘~ 😍' : '嗯嗯嗯，不知道我从哪来~'))))}`,
                text: `啦啦啦，我是卖报的小行家~~ 🤔`,
                html: `<div>
                               <div>啦啦啦，我是卖报的小行家~~ 🤔 
                               <br><br>
    ...
    async deleteReadingArticle(req, res, next) {
        try {
            const id = req.body.id;
            await ReadingArticleModel.findOneAndDelete({ _id: id });
            res.send({
                type: 'success',
                message: `删除成功`,
                status: 1
