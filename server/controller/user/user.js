...
class User {
    ...
    async deleteUser(req, res, next) {
        ...
        try {
            ...
            for (let i of user.following_user) {
                await UserModel.findByIdAndUpdate({ _id: i }, {
                    '$pull': {
                        follower_user: user._id
                    },
                })
            }
            for (let i of user.follower_user) {
                await UserModel.findByIdAndUpdate({ _id: i }, {
                    '$pull': {
                        following_user: user._id
                    },
                })
            }
            await UserModel.findOneAndDelete({ _id: req.body._id });
            await imageArticleModel.deleteMany({ author: req.body._id });
            res.send({
                type: 'success',
                message: '用户状态更新成功',
                status: 1,
    ...
    async getUserInfo(req, res, next) {
        try {
            let user = await UserModel.findOne({ _id: req.body._id, is_actived: true }, { password: 0 }).populate({
                path: 'image_article',
                model: 'ImageArticle',
                populate: {
                    path: 'author',
            ...
            if (!user) {
                user = await AdminModel.findOne({ _id: req.body._id }, { password: 0, token: 0, last_login: 0 });
            }
            if (!user) {
                res.send({
                    type: 'error',
