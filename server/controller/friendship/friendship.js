    ...
    async getFriendShipById(req, res, next) {
        ...
        try {
            ...
            if (!friendship) {
                ...
                let friendInfo = await UserModel.findOne({ _id: req.body.to_user });
                if (!friendInfo) {
                    friendInfo = await AdminModel.findOne({ _id: req.body.to_user });
                }
                res.send({
                    type: 'success',
                    message: `查询关注情况成功`,
                    status: 1,
                    data: {_id: friendInfo._id,isFollowed: true, username: friendInfo.username, description: friendInfo.description, avatar: friendInfo.avatar === undefined ? 'http://mafei.ostqa.cn/default_avatar.png': friendInfo.avatar }
                });
            }
